...
<?php if ( WP_Hotel_Booking::instance()->cart->cart_items_count != 0 ) : ?>
    <div id="hotel-booking-cart">
        ...
        <form id="hb-cart-form" method="post">
            <h3><?php _e( 'Booking Summary', 'wp-hotel-booking' ); ?></h3>
            <table class="hb_table">
                ...
                <th class="hb_night"><?php _e( 'Nights', 'wp-hotel-booking' ); ?></th>
					...
					<?php foreach ( $rooms as $cart_id => $room ): ?>
                        ...
                        <tr class="hb_checkout_item" data-cart-id="<?php echo esc_attr( $cart_id ); ?>">
                            <td<?php echo defined( 'TP_HB_EXTRA' ) && $cart_extra ? ' rowspan="' . ( count( $cart_extra ) + 2 ) . '"' : '' ?>>
                                <a href="javascript:void(0)" class="hb_remove_cart_item" data-cart-id="<?php echo esc_attr( $cart_id ); ?>" title="Click to remove this item." onclick="location.href = 'reservations';">
                ...
                <tr class="hb_sub_total">
                    <td colspan="8"><?php _e( 'Sub Total', 'wp-hotel-booking' ); ?>
                        <small><span class="hb-align-right hb_sub_total_value">
				...
				<?php if ( $advance_payment = $cart->advance_payment ) : ?>
                    <tr class="hb_advance_payment">
                        <td colspan="8"><small><em>
							<?php printf( __( 'Advance Payment ( %s%% of total )', 'wp-hotel-booking' ), hb_get_advance_payment() ); ?>
                            <span class="hb-align-right hb_advance_payment_value"><?php echo hb_format_price( $advance_payment ); ?></span></em></small>
                        </td>
                    </tr>
				<?php endif; ?>
...
<?php else: ?>
    ...
    <div class="hb-message message">
        <div class="hb-message-content">
			<?php _e( 'There is nothing on your booking summary.', 'wp-hotel-booking' ) ?>
        </div>
    </div>
<?php endif; ?>
