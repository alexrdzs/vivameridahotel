				...
				<?php if ( $tos_page_id = hb_get_page_id( 'terms' ) ) { ?>
                    <p>
                        <label>
							...
							<?php printf( __( 'I have read and agree with the ', 'wp-hotel-booking' ) . '<a href="%s" target="_blank">%s</a>', get_permalink( $tos_page_id ), get_the_title( $tos_page_id ) ); ?>
                        </label>
                    </p>
				<?php } ?>
                <p>
                    <button type="submit" class="hb_button"><?php _e( 'Complete Reservation', 'wp-hotel-booking' ); ?></button>
                </p>
