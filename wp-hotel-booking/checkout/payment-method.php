...
<div class="hb-payment-form">
    <div class="hb-col-padding hb-col-border">
        <h4><?php _e( 'Payment Method', 'wp-hotel-booking' ); ?></h4><p>We'll get in touch with you to complete your payment. No need to pay anything right now.</p>
        <ul class="hb-payment-methods">
			<?php $i = 0; ?>
			<?php foreach ( $payment_gateways as $gateway ) : ?>
                <li>
                    <label>
                        <input type="radio" name="hb-payment-method" value="<?php echo esc_attr( $gateway->slug ); ?>"<?php echo ( $i === 0 ) ? '' : '' ?>/>
